...
/* {{{ Prototypes */
static NSMutableArray *generateErrorSpecifiersWithText(id self, NSString *errorText, NSString *errorDetailText);
/* }}} */
...
- (NSMutableArray *)specifiers {
	if(!_specifiers) {
		...
		if(!specifier) {
			...
			return _specifiers = generateErrorSpecifiersWithText(self, errorText, nil);
		}
		...
		if(!_specifiers || [_specifiers count] == 0) {
			...
			_specifiers = generateErrorSpecifiersWithText(self, errorText, nil);
		} else {
...
- (NSArray *)specifiers {
	if(!_specifiers) {
		...
		_specifiers = generateErrorSpecifiersWithText(self, errorText, errorDetailText);
	}
	return _specifiers;
}
